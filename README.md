...
Guild, Raid/Party, and Friends each support an optional per-player recipient subset, session-only (never saved, reset on reload), entirely inside the "Send to:" dropdown itself - no separate control. Clicking one of those three channel rows makes it the active channel and expands its reachable Soundbook users as checkboxes directly beneath it, in the same open list; every reachable member is selected by default. Clicking the already-active channel's row again toggles everyone off or back on, without closing the list - individual members can also be checked/unchecked freely, any number of times, without reopening the menu. A narrowed selection persists for the rest of the session and is never silently grown when someone new becomes reachable - only an explicit check adds them. Switching to a different channel immediately collapses the previous one's member list and discards its subset; only one channel's subset is ever active at a time. A large roster scrolls inside the dropdown rather than growing it indefinitely. A narrowed Guild/Raid send still travels as individual whispers on the wire (Guild/Raid channel messages can't be addressed to specific people), but is received, gated, and acknowledged exactly like a whole-channel broadcast - never mistaken for a Direct send or a plain Friends whisper. Per-sound Default Output overrides and macro `::Target` sends always reach the whole channel, unaffected by this.
