...
- A gear icon at the header's top-left opens **Settings** inside the same window (two-pane layout: General, Playback, Multiplayer, Appearance, Categories, Advanced). Quick Options, Lock, and Close sit at the header's top-right; Raid Admin appears as its own tab when you have the authority to use it.
...
- idle: just the app icon, movable, with small indicators for an active incoming-mute or raid-admin restriction - re-asserted on every zone/instance load (`PLAYER_ENTERING_WORLD`), not just once at login, so it self-heals if it was ever shown before the client's own loading-screen transition had genuinely finished;
...
- right-click the idle icon for Quick Options (mute incoming, lock, muted players, Sound History, Open Settings); "Open Settings" jumps straight to the Settings view in one click - whether Soundbook is currently closed or already open on a different tab - instead of just opening to whichever view it last showed. Dragging its Announcer Size slider shows a live, full-opacity preview of the real Announcer layout (or resizes the real one in place if a sound is already playing), positioned beside the menu instead of behind it, and returns to your saved opacity/size the instant you release it - the menu and slider themselves stay perfectly still under your cursor throughout, unaffected by the Announcer resizing underneath;
- handles overlapping sounds: each additional currently-playing sound gets its own thin cyan vertical marker (with a dark edge behind it for contrast against any fill colour) inside the progress bar, moving left-to-right purely from that ONE sound's own elapsed/duration (0% at its own start, 100% when it finishes) - entirely independent of the primary/displayed sound's own progress, never relative to it, so a marker only ever moves forward and reads correctly even when background sounds have very different durations; a marker disappears the instant its own sound ends, without affecting the primary or any other marker, and a compact queue indicator shows when remote sounds are waiting;
- expands away from whichever screen edge it's closest to, so it always stays fully on-screen; its own tooltip ("Mini Soundbook") does the same, always anchoring to whichever side the Mini Soundbook/Quick Options popup would NOT expand into so it never overlaps it.

Left-click the idle icon (or hover it, if "Open on hover" is enabled) to expand a compact "Mini Soundbook" grid of your favourites for quick access without opening the Main window. The grid's column count follows how many favourites you actually have - 1 column for up to 3, 2 for 4-6, 3 for 7-12, 4 for 13-16, 5 for 17-20 - rather than stretching a handful of favourites across a wide, mostly-empty row; a larger Mini Soundbook Size (Quick Options/Settings) naturally uses fewer, wider columns rather than growing the popup's on-screen footprint further. Managing favourites - adding, removing, renaming - still happens in the Main Soundbook's own Library (see "Main Soundbook" above); the Announcer's popup is a read/play-only shortcut to the same 20 slots. Clicking a favourite plays it without closing the grid - the clicked slot shows its own progress fill for as long as it plays, animated at a smooth, independent frame-rate cadence rather than the coarser steps of the underlying playback poll (most noticeable on short sounds), further favourites stay clickable (with overlap enabled, several can show progress at once), and the grid still collapses normally once you actually move away from it. Each slot also shows a thin gold border while hovered or while its own sound is playing (either alone is enough, and the two never stack into a heavier border).
