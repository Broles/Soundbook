...
## Soundbook 3.0.4 refinement: Online Greeting Announcer layout + display timing

Two related refinements to the Online Greeting Announcer, no changes to detection, Greeting Sound selection/fallback, or any unrelated behaviour.

**1. Text hierarchy flipped (`Announcer.lua`, `RenderPrimary`)**: with a real Greeting Sound, the title is now the sound's own display name (exactly like any ordinary Now Playing sound) and the subtitle is a short `"<Player> is online"` line - the old `"<PLAYER> IS ONLINE"` headline and the `"Greeting Sound: "` prefix crowding the sound name into the subtitle are both gone. The relationship (Friend/Guild/Friend + Guild) still drives the progress bar's own colour exactly as before via `SB.GetChannelColor` - it's simply no longer spelled out as text, which is also what keeps the subtitle short enough to never risk overlap. Since the title now uses the EXACT SAME slot/width any other Now Playing sound name already gets (rather than being squeezed behind "Greeting Sound: " in the narrower subtitle), long sound names are significantly less likely to truncate - with zero geometry/width changes anywhere (`LayoutBanner`, `BANNER_PADDING`, `BANNER_RIGHT_PAD`, the Announcer's own frame width - all untouched), so the Announcer's own width is unchanged, per the explicit requirement. A notification-only greeting (no sound) still falls back to a plain "Soundbook" title, subtitle unchanged in shape.

**2. Fixed 2-second minimum display duration (`Announcer.lua`, `PLAYBACK_PROGRESS_ENDED`)**: a new `GREETING_MIN_DISPLAY_SECONDS = 2` constant is used ONLY for `entry.isGreeting` entries in the existing minimum-display-duration branch, completely independent of the user's configurable "Now Playing Minimum Duration" (`SB.db.settings.announceDuration`, which still applies unchanged to every ordinary sound). `displayDuration = max(2s, known sound duration)` falls straight out of the SAME existing mechanism a normal sound's minimum-duration hold already uses - a Greeting Sound shorter than 2s (e.g. 0.2s or 1.5s) naturally ends first, then gets held/deferred until exactly the 2s mark; a Greeting Sound longer than 2s (e.g. 4s) simply reaches its own natural end AFTER the 2s mark has already passed, so it clears immediately at that real end with no artificial extension. Nothing about the progress bar, compact duration text, or overlap-marker behaviour needed touching - all of that is driven by `entry.duration`/`ValidDuration`, untouched by this change.

**Tests**: new `loader_greeting_layout_duration.lua` - confirms the title is exactly the sound's display name (never containing "IS ONLINE") and the subtitle names the player and reads "... is online" (never "Guild"/"Friend" spelled out, never "Greeting Sound"); confirms the notification-only fallback title/subtitle shape; and, using a `C_Timer.After` spy (the mock's own timer stub doesn't record its delay argument otherwise) to inspect the EXACT deferred-removal delay actually scheduled, confirms a 0.2s and a 1.5s Greeting Sound are each held for precisely `2s - elapsed` (not the test's own deliberately-different `announceDuration` of 7 and 0 respectively - proving the fixed constant is used, not the configurable setting), and that a 4s Greeting Sound clears immediately at its own natural end with zero deferred-removal timers queued (elapsed already past the 2s floor). Also updated `loader_onlinegreetings.lua`'s own banner-text assertion (scenario 7) to match the new title/subtitle shape. Full mock suite re-run (the same three pre-existing, unrelated failures aside - `loader.lua`, `loader_minititle.lua`, `loader_settings.lua`); `luac -p` clean across every `Soundbook.toc`-listed file.

No version bump this round - still 3.0.4, unreleased/unmerged.

## Soundbook 3.0.4: Online Greeting Sound playback regression - investigated, no code defect found

Investigated the reported regression ("Online Greeting notification appears, but the Greeting Sound itself never plays - Announcer falls back to the generic 'Soundbook' title/question-mark icon").

**Traced the full resolution -> playback -> Announcer handoff chain** (`Communication.lua`'s `VerifyAndFireGreeting`/`SB:ResolveGreetingSound`/`SB:GetGreetingSoundFor`/`SB:GetGreetingFallbackSoundFor`, `Announcer.lua`'s `SB:ShowOnlineGreeting`/`AddDisplay`/`RenderPrimary`) line by line against every requirement in the task, and diffed the two most recent commits (`546d966` fallback feature, `7f08670` layout/duration refinement) against the pre-refinement version - neither touches sound resolution, `SB:PlaySound`, or the `soundID`/`played` handoff at all; the layout commit only changes which STRINGS get written into already-correct title/subtitle FontStrings once a real entry exists.

**New test coverage added specifically for the gap this bug report implied**: `loader_greeting_e2e_fallback.lua` - unlike the existing `loader_onlinegreetings.lua` (which only exercises the received-HISTORY Greeting Sound path via a direct `SB:ShowOnlineGreeting` call), this drives a brand-new player's FALLBACK Greeting Sound through the complete REAL production pipeline end to end - a real `GUILD_ROSTER_UPDATE`-style event fires `ScanPresence`, which schedules the real `C_Timer.After(GREETING_VERIFY_DELAY, ...)`-deferred `VerifyAndFireGreeting`, which calls the real `SB:ResolveGreetingSound` (falling through to `SB:GetGreetingFallbackSoundFor`, freshly selecting and persisting from the shortest-favourites pool), which calls the real `SB:ShowOnlineGreeting`, which calls the real `SB:PlaySound` and `AddDisplay` - and asserts the Announcer ends up showing the ACTUAL resolved sound's real name/icon, never the generic "Soundbook" fallback title. This passed on first run against the current code, exactly as every other test covering this feature already did.

**Conclusion**: no code defect was found anywhere in the resolution/playback/Announcer-handoff chain - every scenario in the task's own Verify list (existing history, persisted fallback, fresh fallback selection, Play Greeting Sound off, no valid sound, short/long sound display timing, no network traffic) already passes against the current implementation, both via existing tests and this round's new end-to-end one.

**Most likely real-world explanation, not a code bug**: `SB:ShowOnlineGreeting` already has an intentional, documented fallthrough - "Sound failed to play (muted mid-flight, combat/encounter-gated, missing file, etc.) - fall through to the notification-only toast" - and this repo has a KNOWN, previously documented case matching that exactly: `Core.lua`'s own 3 default fresh-install Favourites include `"Dry Fart"`, whose audio file this very QA report already documented elsewhere as truncated/corrupted on disk (measures ~1.0s of real audio against its own registered ~2.2s duration). On a newer install whose Favourites still include that default, a Greeting Sound fallback landing on that specific sound would legitimately fail to play via `SB:PlaySound` (unrelated to any Online Greetings code) and correctly fall back to the notification-only toast - reproducing the exact reported symptom without any bug in the greeting logic itself.

**Change made**: added one `SB:Debug` call in `Announcer.lua`'s `SB:ShowOnlineGreeting` fallthrough branch, so a future occurrence of "notification shown but no sound" is directly traceable (which player, which soundID) with Debug Mode on, instead of a silent mystery - `SB:PlaySound` itself already logs the specific reason (muted/combat/encounter/missing file) via its own existing `SB:Debug` calls.
