...
local function RefreshLibraryImpl()
    ...
    -- anchor chain (main -> toolbar -> tagFilterBar -> libraryScroll.
    -- scroll) rather than a single hop off `main` directly
...
-- "active side marker" + a soft outer glow), inactive stays clearly
-- quieter, and a disabled (unavailable) bucket stays readable but
-- obviously unusable. Shared by the broadcast tabs AND the utility
-- (Admin/Settings) tabs below - same visual family, just a different
-- colour and a different notion of "active".
------------------------------------------------------------------------
...
local function CreateAttachedTab(parent, color, refreshFn, noOwnBorder)
    ...
    -- would be) rather than tinting the whole tile.
    local accent = btn:CreateTexture(nil, "ARTWORK")
    accent:SetPoint("TOPLEFT", 1, -1)
    accent:SetPoint("BOTTOMLEFT", 1, 1)
    accent:SetWidth(3)
    accent:SetTexture("Interface\\Buttons\\WHITE8X8")
    accent:SetVertexColor(color[1], color[2], color[3], 0.85)
    btn.accent = accent
    ...
    btn:SetScript("OnEnter", function(self) self:SetBackdropColor(0.04, 0.10, 0.20, 0.92) end)
...
------------------------------------------------------------------------
-- Lower-right utility tabs - Admin (only visible to the current Raid
-- Leader/Assist or Party Leader, AdminPanel.lua) and Settings (always
-- available). Explicit request: back to attached right-side tabs (like
-- the pre-3.0 2.7.x book), same visual family as the broadcast tabs above
-- but with a clear vertical gap - anchored independently to the
-- Soundbook's own BOTTOMRIGHT corner instead of chained below them, and a
-- neutral gold accent instead of a channel colour, so the two groups
-- never get visually confused with each other despite matching chrome.
------------------------------------------------------------------------
...
local function BuildMainFrame()
    ...
    ------------------------------------------------------------------
    -- Send-to selector with an integrated per-player recipient subset
    -- (explicit request: the whole interaction lives inside this ONE
    -- dropdown - no second control or panel attached beside/below it).
    -- Top rows: All / Self / Friends / Guild / Raid-Party. Clicking a
    -- Friends/Guild/Raid-Party row (see BUCKET_ORDER below) makes it the
    -- active channel and expands its member checkboxes directly under it,
    -- inside the SAME open list - Theme.CreateDropdown's row
    -- opt.onRowClick/opt.checkbox and dd:RefreshList/SetLabelText (see
    -- Theme.lua) make a row fully own its own click behaviour and let the
    -- open list rebuild in place instead of closing. Reuses the exact
    -- same session-only subset state Communication.lua already exposes -
    -- no parallel discovery/transport.
    ------------------------------------------------------------------
    local BUCKET_LABEL = { FRIENDS = "Friends", GUILD = "Guild" }
    local BUCKET_ORDER = { "FRIENDS", "GUILD", "RAID" }
    local MEMBER_INDENT = 16

    local function RaidPartyLabel()
        if IsInRaid and IsInRaid() then return "Raid" end
        if IsInGroup and IsInGroup() then return "Party" end
        return "Raid/Party"
    end
    local function BucketLabel(bucket)
        return bucket == "RAID" and RaidPartyLabel() or BUCKET_LABEL[bucket]
    end

    local function ActiveBucket()
        local target = SB.db.settings and SB.db.settings.defaultOutputTarget
        if target == "FRIENDS" or target == "GUILD" or target == "RAID" then return target end
        return nil
    end

    -- "Send to: X" closed-chip text, independent of the option list's own
    ...
    local function BuildSendToOptions()
        ...
        for _, bucket in ipairs(BUCKET_ORDER) do
            ...
            table.insert(opts, {
                ...
                onRowClick = function()
                    if active then
                        -- "Clicking the active channel again toggles all
                        -- its members off/on" - explicit requirement.
                        SB.ToggleAllChannelMembers(bucket)
                    else
                        SB.db.settings.defaultOutputTarget = bucket
                        SB.ResetChannelSubsetsExcept(bucket)
                        SB.ActivateChannelSubset(bucket)
                    end
                    RefreshSendToLabel()
                    defaultOutputDD:RefreshList()
                    SB:Fire("OUTPUT_SELECTION_CHANGED")
                end,
            })
                ...
                if #memberRows > 0 then
                    for _, row in ipairs(memberRows) do
                        ...
                        table.insert(opts, {
                            ...
                            value = "MEMBER:" .. name, indent = MEMBER_INDENT,
                            checkbox = true, checked = row.selected,
                            onRowClick = function()
    ...
    defaultOutputDD:SetOptions(BuildSendToOptions())
    defaultOutputDD:SetOptionsProvider(BuildSendToOptions)
    -- Colours a channel row by its own SB.CHANNEL_COLOR (matching every
    -- other Guild/Raid/Friends label in the addon), bolds/brightens the
    -- currently active one (explicit requirement: "visually selected"),
    -- and dims a disabled placeholder row. Member rows use the plain
    -- default text colour - their own checkbox glyph already carries the
    -- selected/unselected state (explicit requirement: "using the
    -- existing Soundbook checkbox/toggle language").
    defaultOutputDD:SetRowFont(function(text, opt)
        if opt.disabled then
            text:SetTextColor(unpack(SB.Theme.TEXT_DIM))
        elseif opt.checkbox then
            text:SetTextColor(unpack(SB.Theme.TEXT))
        else
            local color = SB.CHANNEL_COLOR[opt.value]
            if color then
                if opt.isActiveChannel then
                    text:SetTextColor(color.r, color.g, color.b)
                else
                    text:SetTextColor(color.r * 0.55, color.g * 0.55, color.b * 0.55)
                end
            else
                text:SetTextColor(unpack(SB.Theme.TEXT))
            end
    ...
    SB.Theme.AttachTooltip(defaultOutputDD.button, "Send to",
        "Where a regular click sends a sound by default. Open Friends/Guild/Raid-Party to choose exactly who within that channel receives it. A sound's own per-sound Default Output (Edit Sound) overrides this when set.")
    ...
    -- parallel refresh trigger. dd:RefreshList is itself a no-op while
    -- the list is closed, so this is always safe to call unconditionally.
...
function SB:ShowMainWindow()
    ...
    -- hop anchor chain (main -> toolbar -> tagFilterBar -> libraryScroll.
    -- scroll - see RefreshLibrary's own comment) hadn't fully resolved
    -- widths yet on this first synchronous pass. Cheap (one extra layout
    -- pass) and always safe to repeat.
